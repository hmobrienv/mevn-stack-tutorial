<template>
  <div class="posts">
    <h1>Posts</h1>
    <div v-if="services.length > 0" class="table-wrap">
      <div>
      ...
      <table>
        ...
        <tr v-for="service in services" :key="service._id">
          ...
          <td align="center">
            <router-link
              v-bind:to="{ name: 'EditPost', params: { id: service._id } }"
            >
              Edit
            </router-link>
            <a href="#" @click="deletePost(service._id)">Delete</a>
          </td>
        </tr>
      </table>
    </div>
...
export default {
  ...
  methods: {
    async getPosts() {
      ...
      this.services = response.data.posts;
